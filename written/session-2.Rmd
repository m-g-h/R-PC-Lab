...
In this section we will cover the basic features of probability distributions, also called the *moments* of the distribution.

## Central Tendency: The Expected Value

The most commonly used feature of a probability distribution is the *expected value*, or the **first moment**. It splits the probability under the *pdf* in half.
...
The central tendendcy tells us, around which value the outcomes of the random variable cluster, but it is also important to get a measure on how far they *spread around the mean*. The variance and standard deviation are measures for this. The variance is also referred to as the **second moment**.
...
- Variance of a sum of random variables, if they are i.i.d:

$$ Var\left[ \sum_{i=1}^n a_iX_i \right] = \sum_{i=1}^n Var [a_iX_i] = \sum_{i=1}^n a_i^2 Var[X_i] $$
...
- The population correlation coefficient is usually denoted by *rho*: $\rho$ or $\rho_{XY}$ instead of $Corr[X,Y]$

# Hypothesis Testing

Using the properties of random variables and their distributions from above, we now can come to a very useful application: hypothesis testing.

For example let us say that we are interested in the income of ZU Students, and know the following:

- The income is distributed normally with mean $1000$ and standard deviation $200$
