...
$$econgrade = \beta_0 + \beta_{1statsgrade}+ \epsilon$$
...
```
We see that the students with better statistics grades also achieved better econometrics grade. In fact, we could calculate the line's slope and then we'd know by how much the econometrics grade increases if the statistics grade is increased by 1. 
But to infer from this model to the general population and to find out whether or not this could be a causal relationship, we need to know more than this.

## Law of Iterated Expectations
To proceed, we have to have a short look at the Law of Iterated Expectations (LEI). It states that the Expectation of $y$ given $x$ is equal to the expectation of $y$:
...
Now suppose that every time a dart is thrown, someone watching in the other room tells us whether the dart landed in the upper or lower half of the board. Let this be another random variable $X$. By the information of $X$ we can improve our guess for the landing spot - if $X = upper$, then our best guess for $Y$ might be the center of the upper half of the board, $\mathbb{E}\left[Y|X = up\right] = c^{upper}$, if $X = lower$, then our best guess for $Y$ would be the center of the lower half of the board, $\mathbb{E}\left[Y|X = low\right] = c^{lower}$
...
<span style="color:red">INCLUDE PROOF HERE</span>This can also be mathematically proven by:

## Causal interpretation
For a regression to be more than a simple correlation, one has to make assumptions how $x$ is related to the error term $\epsilon$. For causal interpretations, $x$ must not include information about $\epsilon$, that is, they must not be correlated. Mathematically speaking: $$\mathbb{E} \left[\epsilon|x\right] = 0$$
If this holds, we have **exogeneity**. In the example with the statistics and econometrics grade, we obviously do not have exogeneity, as the statistics grade is for example also highly correlated with the number of hours spend studying, which the model does not explicitly include.


Following from this and the Law of Iterated Expectations: $$\mathbb{E} \left[\epsilon|x\right] = \mathbb{E}\left[\epsilon\right] = 0$$, therefore $$cov\left[x, \epsilon\right] = \mathbb{E}\left[x\epsilon\right] = 0$$
Effectively, if the expected value of the error term is zero, the model quite accurately predicts the outcome.
