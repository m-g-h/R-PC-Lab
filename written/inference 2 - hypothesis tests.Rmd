...
Look up the critical $z-value$ for $1-10\% = 90\%$, e.g. in this [table](http://www.z-table.com/).
$$ z_{1-\alpha} = 1.65 $$

If we now compare our test statistic against the critical value, we can see that it is greater:
...
- If the $H_0$ is true, samples with a $Z_{inc}$ in the dark grey area only occur $10\%$ of the time
- In conclusion, we have some evidence against the our initial assumption that mean student income is at most $940 €$
- This means that we can be optimistic that our sample is not one of th e$10 \%$ of outliers in the world of $H_0$, but that the $H_0$ is wrong

Generally we can make the statement: "we can reject the null hypothesis that mean ZU student income is less than $940€$ at a significance level of $10\%$"
...
In our example, we could not only reject the $H_0$ at the $10\%$ level, but also at the $5\%$ level:
...
```{r}
t.test(sample_2, mu = 940, alternative = "greater")
```
